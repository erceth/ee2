<div class="col-xs-12">
    <div class="projects page">
        <div class="col-xs-12">
            <h2>Projects</h2>
        </div>
        <div class="all-projects col-xs-12">

            <div class="project col-xs-12 col-sm-6 col-md-4 col-lg-3">
            	<a href="/web-crawler">
	            	<div class="project-wrapper">
		            	<img class="project-img" src="../img/webcrawler_screenshot.png">
		            	<div class="blurb">
		            		<div class="title"><h3>Web Crawler</h3></div>
		            		<p class="short-description"><?php echo $this->render($web_crawler_short_description,$this->mime,get_defined_vars()); ?><p>
		                    <p class="medium-description"><?php echo $this->render($web_crawler_medium_description,$this->mime,get_defined_vars()); ?></p>
		                </div>
		            </div>
		        </a>
            </div>


            <div class="project col-xs-12 col-sm-6 col-md-4 col-lg-3">
            	<a href="/jsflags">
	            	<div class="project-wrapper">
		            	<img class="project-img" src="../img/tanks2.png">
		            	<div class="blurb">
		            		<div class="title"><h3>JSFlags</h3></div>
		                    <p class="short-description"><?php echo $this->render($jsflags_short_description,$this->mime,get_defined_vars()); ?></p>
		                    <p class="medium-description"><?php echo $this->render($jsflags_medium_description,$this->mime,get_defined_vars()); ?></p>
		                </div>
		            </div>
		        </a>
            </div>


            <div class="project col-xs-12 col-sm-6 col-md-4 col-lg-3">
            	<a href="/sms-tracker">
	            	<div class="project-wrapper">
		            	<img class="project-img" src="../img/google-sheets.png">
		            	<div class="blurb">
		            		<div class="title"><h3>SMS Attendance</h3></div>
		            		<p class="short-description"><?php echo $this->render($sms_tracker_short_description,$this->mime,get_defined_vars()); ?></p>
		                    <p class="medium-description"><?php echo $this->render($sms_tracker_medium_description,$this->mime,get_defined_vars()); ?></p>
		                </div>

		                
		            </div>
		        </a>
            </div>

            <div class="project col-xs-12 col-sm-6 col-md-4 col-lg-3">
            	<a href="/freedom-festival">
	            	<div class="project-wrapper">
		            	<img class="project-img" src="../img/provo_freedom_festival.png">
		            	<div class="blurb">
		            		<div class="title"><h3>Freedom Festival</h3></div>
		            		<p class="short-description"><?php echo $this->render($freedom_festival_short_description,$this->mime,get_defined_vars()); ?></p>
		                    <p class="medium-description"><?php echo $this->render($freedom_festival_medium_description,$this->mime,get_defined_vars()); ?></p>
		                </div>

		                
		            </div>
		        </a>
            </div>


        </div>
    </div>
</div>